...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorker, roomId, authKey })
	{
		...
		logger.info('create()xdd [mediasoupRouter:%s]', JSON.stringify(mediasoupRouter));
	...
	{
		logger.info(
			'logStatus() [roomId:%s, protoo Peers:%s, mediasoup Transports:]',
			this._roomId,
			this._protooRoom.peers.length,
			); // NOTE: Private API.
			logger.info(
				JSON.stringify(this._mediasoupRouter)
			);
		// logger.info(
